...
export class Fixture {
  constructor() {
    ...
    this.stubComponent(APIProvider)
      .callFake(this.apiProviderFixture_.Component);
  ...
  render() {
    const { container } = render(<App key={Math.random()} config={this._config} />);
      ...
      const Impl = useMemo(
        () => {
          if (this._implementation) {
            return forwardRef((props, ref) => this._implementation(props, ref));
          }
          return Component;
        },
        // eslint-disable-next-line react-hooks/exhaustive-deps
...
class APIProviderFixture {
  constructor() {
    this._comp = ({ children }) => {
      ...
      const autoSaveById = useCallback(() => jasmine.createSpy('autoSaveById'), []);
      const saveStoryById = useCallback(() => jasmine.createSpy('saveStoryById'), []);
      const deleteStoryById = useCallback(() => jasmine.createSpy('deleteStoryById'), []);
      ...
      const uploadMedia = useCallback(() => jasmine.createSpy('uploadMedia'), []);
      const updateMedia = useCallback(() => jasmine.createSpy('updateMedia'), []);

      const getLinkMetadata = useCallback(() => jasmine.createSpy('getLinkMetadata'), []);

      const getAllStatuses = useCallback(() => jasmine.createSpy('getAllStatuses'), []);
      const getAllUsers = useCallback(() => jasmine.createSpy('getAllUsers'), []);
...
function actPromise(value) {
  ...
  return {
    then(callback, errback) {
      ...
      return actPromise(promise.catch((reason) => act(async () => errback(reason))));
    },
    finally(callback) {
      return actPromise(
